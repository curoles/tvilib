...
void arrays_{{name}}(T* dst_array, const T* array1, const T* array2, std::size_t nr_elem)
{
    ...
    if (a.nr_tail_elem) {
        ...
        VT dv = {{vop}}(v1, v2); //FIXME use mask
        tvx::vst((VT*)&dst_array[a.nr_chunks*a.NR_ELEM], mask, dv);
    }
}

template<typename T, std::size_t VLEN = 512>
void arrays_{{name}}(std::span<T> dst_array, std::span<const T> array1, std::span<const T> array2)
{
    assert(array2.size() >= array1.size());
    assert(dst_array.size() >= array1.size());

    vil::arrays_{{name}}<T,VLEN>(dst_array.data(), array1.data(), array2.data(), array1.size());
}
